...
main :: proc() {
	...
	for char in char_positions {
		...
		for i in 0 ..< positions_count {
			blk: for j in 0 ..< positions_count do if j != i {
				new_antinode_loc := char_positions[char][i] + (char_positions[char][i] - char_positions[char][j])

				if new_antinode_loc.x < 0 || new_antinode_loc.x >= width || new_antinode_loc.y < 0 || new_antinode_loc.y >= height do continue
